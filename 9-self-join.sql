...
SELECT first.num, first.company, first.name as transfer, second.num, second.company
FROM (
    ...
    WHERE stopa.name = 'Craiglockhart' AND stopb.name <> 'Craiglockhart') first
JOIN (
    ...
    WHERE stopc.name = 'Lochend' AND stopd.name <> 'Lochend') second
